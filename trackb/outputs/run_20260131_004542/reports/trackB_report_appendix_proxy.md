...
- **Benchmark performance (WM-811K)**: Pattern classification test accuracy **86.4%**, macro-F1 **0.69** (9 classes); see table below.
...
- Why proxy: Different data source; no same-wafer GT.
- **Benchmark performance (Carinthia)**: Defect classification triage distribution (A/B/C/D) and mean severity; see table below (normalized units only, no currency).
